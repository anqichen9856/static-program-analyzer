...
CallsClause::CallsClause(string left, string right) : RelClause(left, right) {}

void CallsClause::validate(unordered_map<string, DesignEntity> map) {
...
bool CallsClause::evaluate(Query *query, QueryResult* query_res) {
    unordered_map<string, DesignEntity> syns = query->synonyms;
    bool result = false;

    if (isName(left) && isName(right)) {
        result = PKB::getInstance()->isCalls(strip(left), strip(right));
    } else if (isName(left) && isWildcard(right)) {
        vector<string> res = PKB::getInstance()->getCalls(strip(left));
        if (res.size() > 0) {
            result = true;
        }
        ...
        vector<string> res = PKB::getInstance()->getCalledBy(strip(right));
        if (res.size() > 0) {
            result = true;
        }
        ...
        for (auto i : syn_procedure) {
            if (PKB::getInstance()->getCalledBy(i).size() > 0) {
                res.push_back(i);
            }
        }
        if (query_res->isInTable(right)) {
        ...
        for (auto i : syn_procedures) {
            if (PKB::getInstance()->getCalls(i).size() > 0) {
                res.push_back(i);
            }
        }
        if (query_res->isInTable(left)) {
        ...
        if (query_res->isInTable(left) && query_res->isInTable(right)) {
            ...
            for (auto i : left_procedures) {
                for (auto j : right_procedures) {
                    if (PKB::getInstance()->isCalls(i, j)) {
                        pairs.push_back({ i, j });
                    }
                }
            }
...
string CallsClause::getClauseName() {
    return "Calls";
}

int CallsClause::getRank() {
    return 2;
}
